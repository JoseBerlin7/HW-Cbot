...
class main_1:
  def __init__(self):
      ...
      self.embedding, self.model, self.file_paths = self.preload()
  ...
  def preload(self):
      ...
      file_contents = [open(file_path, "r").read()[:100] for file_path in file_paths]
      if len(file_contents)<1:
        print("No course files found")
      model = SentenceTransformer("sentence-transformers/all-MiniLM-l6-v2")
      file_embeddings = model.encode(file_contents)
      return file_embeddings, model, file_paths

  # To get the most relevant file for course selection
  def get_most_rel_filename(self, query):
      query_embedding = self.model.encode(query)
      similarity_scores = util.pytorch_cos_sim(query_embedding, self.embedding)
      most_similar_file_index = similarity_scores.argmax().item()
      most_similar_file_name = os.path.basename(self.file_paths[most_similar_file_index])
